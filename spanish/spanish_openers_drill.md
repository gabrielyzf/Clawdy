# Spanish Sentence Openers — Drill Tracker

_Append-only. Status progression: new → learning → active → mastered._
_Phase 1 priority (first 4 weeks): **opinion + contrast**. Expand to all 8 categories afterward._

## Format per entry

```
### [Spanish opener]
- category: opinion | contrast | agree | disagree | clarify | hypothesize | emphasize | summarize
- gloss_en: ...
- gloss_cn: ...
- status: new | learning | active | mastered
- times_used: N
- last_used: YYYY-MM-DD or null
- next_review: YYYY-MM-DD
- example: "Gabriel's own sentence using it (filled in once he uses it)"
- notes: optional usage warnings, register, B2 nuance
```

---

## Phase 1 — Priority categories (opinion + contrast)

### Category 1 — Express opinion

#### Desde mi punto de vista...
- category: opinion
- gloss_en: From my point of view...
- gloss_cn: 从我的角度来看……
- status: learning
- times_used: 1
- last_used: 2026-06-01
- next_review: 2026-06-04
- example: (already used in Barça reading task — confirm reuse)
- notes: Frame for opinion essays + investment thesis. Highest leverage opener for Gabriel.

#### A mí me parece que...
- category: opinion
- gloss_en: It seems to me that... / I think that...
- gloss_cn: 我觉得……（更口语、更软）
- status: learning
- times_used: 1
- last_used: 2026-06-02
- next_review: 2026-06-05
- example: (pending Gabriel's reply — Sevilla travel roleplay)
- notes: Softer than "yo creo". Subjunctive trigger when negated: *no me parece que sea...*

#### Yo diría que...
- category: opinion
- gloss_en: I would say that...
- gloss_cn: 我会说……（带点缓冲）
- status: learning
- times_used: 1
- last_used: 2026-06-04
- next_review: 2026-06-07
- example: (pending Gabriel's reply — café conversation on markets)
- notes: Conditional softener. Polite, hedged opinion. Great for investment thesis — softer than "yo creo", more confident than "a mí me parece".

#### Lo que pienso es que...
- category: opinion
- gloss_en: What I think is that...
- gloss_cn: 我的想法是……
- status: learning
- times_used: 1
- last_used: 2026-06-09
- next_review: 2026-06-12
- example: (pending Gabriel's reply — Tue vocab+drills, escapada a Granada)
- notes: Strong frame for structured opinion. Common in writing. Hoy = primer uso forzado en tarea de planificación de viaje.

### Category 2 — Contrast / pivot

#### Por un lado... por otro...
- category: contrast
- gloss_en: On one hand... on the other...
- gloss_cn: 一方面……另一方面……
- status: learning
- times_used: 2
- last_used: 2026-06-07
- next_review: 2026-06-10
- example: (pending Gabriel's reply — Sun light review, retrospectiva semanal)
- notes: Backbone for any balanced argument. Pairs perfectly with *desde mi punto de vista*. Hoy se reactiva como par estructurador para la retrospectiva.

#### Sin embargo,...
- category: contrast
- gloss_en: However,...
- gloss_cn: 然而……
- status: learning
- times_used: 1
- last_used: 2026-06-10
- next_review: 2026-06-13
- example: (pending Gabriel's reply — Wed roleplay café sobre tesis IA/cripto)
- notes: B2 register, más escrito que hablado pero también funciona en habla cuidada. Pivote más fuerte que *pero*. Hoy = primer uso forzado en roleplay de mercado.

#### Aun así,...
- category: contrast
- gloss_en: Even so,... / Still,...
- gloss_cn: 即便如此……
- status: learning
- times_used: 1
- last_used: 2026-06-05
- next_review: 2026-06-08
- example: (pending Gabriel's reply — Camp Nou matchday listening + spoken summary)
- notes: Concession + pivot. Useful in football/investment debate. Funciona muy bien después de admitir un punto del rival: "Tienes razón en X. Aun así, Y".

#### Dicho esto,...
- category: contrast
- gloss_en: That said,...
- gloss_cn: 话虽如此……
- status: learning
- times_used: 1
- last_used: 2026-06-06
- next_review: 2026-06-09
- example: (pending Gabriel's reply — Sat debate IA vs. analistas junior, tertulia de café)
- notes: Smooth pivot in extended argument. Very B2. Perfect after conceding a point: "Tienes parte de razón en X. Dicho esto, Y."

#### Ahora bien,...
- category: contrast
- gloss_en: That being said... / Now,...
- gloss_cn: 不过呢……（书面/正式）
- status: new
- times_used: 0
- last_used: null
- next_review: tbd
- notes: More formal pivot. Common in op-ed writing.

---

## Phase 2 — Expansion (weeks 5+)

### Category 3 — Agree / partial agree
- Estoy de acuerdo en que... — I agree that...
- En parte sí, pero... — Partly yes, but...
- Hasta cierto punto... — Up to a point...
- No te quito la razón, pero... — You're not wrong, but...

### Category 4 — Disagree (soft)
- No estoy del todo de acuerdo... — I don't fully agree...
- Yo lo veo de otra manera... — I see it differently...
- Yo más bien diría que... — I'd rather say that...

### Category 5 — Clarify / restate
- Lo que quiero decir es que... — What I mean is...
- O sea... — I mean... / That is...
- Es decir... — That is to say...
- Mejor dicho... — Or rather...
- En otras palabras... — In other words...

### Category 6 — Hypothesize / condition
- Si tuviera que elegir... — If I had to choose...
- Suponiendo que... — Supposing that... (subjunctive)
- En caso de que... — In case... (subjunctive)
- A no ser que... — Unless... (subjunctive)

### Category 7 — Emphasize
- Lo importante aquí es... — What's important here is...
- Lo que más me llama la atención es... — What strikes me most is...
- No se trata de X, sino de Y... — It's not about X, but about Y...
- Lo cierto es que... — The truth is that...

### Category 8 — Summarize / close
- Al fin y al cabo... — At the end of the day...
- En definitiva... — Ultimately...
- A fin de cuentas... — In the end...
- Resumiendo... — To sum up...

---

## Drill log (append daily)

_Format: `YYYY-MM-DD — opener — context — Gabriel's sentence — accuracy notes`_

2026-06-02 — A mí me parece que... — Sevilla travel roleplay (opinion on neighborhoods + tapas) — pending — pending
2026-06-03 — Sin embargo,... — Writing-gym opinion piece on AI agents wave — pending — pending
2026-06-04 — Yo diría que... — Café roleplay sobre mercados / IA / cripto — pending — pending
2026-06-05 — Aun así,... — Listening + resumen sobre partido Barça (radio post-partido) — pending — pending
2026-06-06 — Dicho esto,... — Sat debate (tertulia de café): ¿la IA sustituye a los analistas junior o los potencia? — pending — pending
2026-06-07 — Por un lado... por otro... — Sun light review: retrospectiva de la semana (trabajo + mercados + Barça) — pending — pending
2026-06-09 — Lo que pienso es que... — Tue vocab+drills: escapada a Granada (travel/Spain) — pending — pending
2026-06-10 — Sin embargo,... — Wed conversation/roleplay: charla de café sobre tesis de inversión (cripto / IA / bolsa) — pending — pending
